...
function AdminApp() {
  ...
  const [original, setOriginal] = React.useState(INITIAL_CONTENT);
  ...
  const dirty = React.useMemo(() => JSON.stringify(content) !== JSON.stringify(original), [content, original]);
  ...
  const save = () => {
    setOriginal(content);
    setToast({ msg: "Saved & deploying", tone: "green" });
  ...
  return (
    <div style={{
      ...
      <TopBar dirty={dirty} onSave={save} onDiscard={discard} onPreview={()=>setPreview(true)} onPalette={()=>setPalette(true)} onHelp={()=>setCheatsheet(true)}/>
...
// ─────────────────────────────────────────────────────────────
function TopBar({ dirty, onSave, onDiscard, onPreview, onPalette, onHelp }) {
  return (
    <div style={{
      gridColumn: "1 / -1",
      ...
      <Btn primary onClick={onSave} disabled={!dirty}><span style={{display:"inline-flex",alignItems:"center",gap:6}}>{Ic.save} Save & deploy</span></Btn>
