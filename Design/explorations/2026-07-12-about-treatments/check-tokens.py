...
appears ONLY as the value of an --at-<slug>-* declaration inside the sheet's
own :root { } block. Raw hex/rgb/hsl anywhere else — including inside :root
but not bound to an --at-* custom property (e.g. a literal `background:
...
bound to an --at-* token. Replaced with a strip-then-scan pass: every
`--at-<slug>-*: <color>;` declaration is removed from the declared text,
...
# an --at-<slug>-* custom property. Strip every valid `--at-*: <color>;`
...
if bad_decl:
    problems.append(
        f"{len(bad_decl)} raw color(s) inside :root not bound to an --at-* declaration:"
    )
...
print(f"PASS {sys.argv[1]}: colors confined to declared --at-* tokens.")
