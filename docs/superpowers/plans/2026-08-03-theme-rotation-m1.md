...
- **The extraction must be VISUALLY identical (amended 2026-08-03, Este ruling: Option B).** Byte-identical HTML is impossible once theme CSS moves into a linked `tokens.css`; the original byte-identical wording contradicted the extraction itself. The real gate: after T2, `index.html` differs from `origin/main` ONLY by the inline theme-CSS block being replaced with a `<link>` to the theme's tokens.css, and the rendered page is visually identical — verified by Playwright screenshot comparison plus computed-style spot checks at 1440/768/390. Any other diff is a DESIGN FINDING (report it, stop, do not paper over it). Additionally: `index_changed` MUST compare the render against the DESTINATION file, never against the shell that was read as source, or index.html silently rots while --check reports clean.
