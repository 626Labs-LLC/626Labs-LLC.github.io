...
- [ ] **Step 2: Reading.** `reading.css` dresses every `.lnt-*` selector PB's `reading.css` dresses (list them from PB's file; the doctor requires the seven vocabulary classes as selectors). Slate broadsheet idiom: the record rails become folio columns, the frontispiece a masthead, pull quotes in serif italic with a hairline. `reading.html` is NOT the archetype shell the doctor renders (the doctor opens `about.html` for the reading archetype); it is the live Field Note template `render_story_pages` reads for the ACTIVE theme, and Field Notes are theme-agnostic by ruling. Leave it byte-identical to Phosphor Blueprint's. (Corrected 2026-09-04 after Task 4 caught the error.) Verify about.html with the slate dress linked over its own style (the picker's action) renders every element and no `.lnt-*` element is left unstyled.
